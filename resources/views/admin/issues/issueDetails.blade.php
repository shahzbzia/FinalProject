			...
			<div class="h-100 w-full flex items-center justify-center bg-teal-lightest font-sans">
				<div class="bg-white rounded shadow p-6 m-4 w-full lg:w-full">
			        ...
			        <div class="mb-4">
			            ...
			            <div class="flex mb-1 items-center">
			                <p class="w-full text-grey-darkest"><strong>Description: </strong>{{ $issue->description }}</p>
			            </div>
			          	<div class="flex mb-1 items-center">
			                <p class="w-full text-grey-darkest"><strong>Reported at: </strong>{{ $issue->created_at }}</p>
			            </div>
			            <div class="flex mb-1 items-center">
			                <p class="w-full text-grey-darkest"><strong>Resolved at: </strong>{{ ($issue->resolved_at) ? $issue->resolved_at : 'Not yet resolved' }}</p>
			            </div>
			        </div>

			        @if (!$issue->resolved_at)
			        	<a href="{{ route('issue.resolved', $issue->id) }}" class="px-3 py-1 text-green-600 font-semibold border-2 border-green-600 rounded-lg hover:bg-green-600 hover:text-black hover:no-underline hover:text-white ">Mark as Resolved</a>
			        @else
			        	<a href="{{ route('issue.unresolved', $issue->id) }}" class="px-3 py-1 text-blue-600 font-semibold border-2 border-blue-600 rounded-lg hover:bg-blue-600 hover:text-black hover:no-underline hover:text-white ">Set back as un-resolved</a>
			        @endif
